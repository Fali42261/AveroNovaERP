...
public sealed class LocalCustomerService : ICustomerService
{
    ...
    public LocalCustomerService(IDbContextFactory<LocalAppDbContext> dbFactory, IAppSessionContext session)
    {
    ...
    public async Task<(bool Ok, string? Error)> DeleteAsync(Guid id)
    {
        ...
        LocalSyncQueueWriter.Enqueue(db, "Customer", row.Id, row.CompanyId, SyncOperation.Delete, new { row.Id }, DateTime.UtcNow);
        await db.SaveChangesAsync();
        return (true, null);
    }
    ...
    private static CustomerModel Map(LocalCustomerEntity row)
        => new()
        {
            LocalId = row.Id,
    ...
    private static LocalCustomerEntity ToEntity(CustomerModel model, DateTime now)
        => new()
        {
            Id = model.LocalId,
    ...
    private static void Apply(LocalCustomerEntity row, CustomerModel model, DateTime now)
    {
        ...
        row.Email = model.Email.Trim();
        row.Phone = model.Phone.Trim();
        row.Address = model.Address;
        row.City = model.City;
        row.Country = model.Country;
        row.TaxNumber = model.TaxNumber;
        row.Notes = model.Notes;
        row.Status = (int)model.Status;
        row.OutstandingBalance = model.OutstandingBalance;
        row.TotalPurchases = model.TotalPurchases;
        row.UpdatedAtUtc = now;
        row.SyncError = null;
    }

    private static object Payload(LocalCustomerEntity row)
        => new { row.Id, row.CompanyId, row.Name, row.Email, row.Phone };
