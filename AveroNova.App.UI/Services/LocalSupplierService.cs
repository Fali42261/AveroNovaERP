...
public sealed class LocalSupplierService : ISupplierService
{
    ...
    public async Task<List<SupplierModel>> GetAllAsync(Guid companyId)
    {
        ...
        return (await db.Suppliers.AsNoTracking().Where(x => x.CompanyId == companyId).OrderBy(x => x.Name).ToListAsync()).Select(Map).ToList();
    }
    ...
    public async Task<(bool Ok, string? Error)> CreateAsync(SupplierModel model)
    {
        ...
        await using var db = await _factory.CreateDbContextAsync();
        if (await db.Suppliers.AnyAsync(x => x.CompanyId == model.CompanyId && x.Name.ToLower() == model.Name.Trim().ToLower())) return (false, "A supplier with this name already exists.");
    ...
    public async Task<(bool Ok, string? Error)> UpdateAsync(SupplierModel model)
    {
        ...
        if (await db.Suppliers.AnyAsync(x => x.CompanyId == model.CompanyId && x.Id != model.LocalId && x.Name.ToLower() == model.Name.Trim().ToLower())) return (false, "A supplier with this name already exists.");
    ...
    public async Task<(bool Ok, string? Error)> DeleteAsync(Guid id)
    {
        ...
        if (await db.Purchases.AnyAsync(x => x.CompanyId == row.CompanyId && x.SupplierId == id)) return (false, "Supplier cannot be deleted because purchases reference it.");
    ...
    private static object Payload(LocalSupplierEntity x) => new { x.Id, x.CompanyId, x.Name, x.Email, x.Phone, x.Address, x.TaxNumber, x.Notes, x.IsActive, SyncVersion = 1L };
