...
public sealed class LocalPurchaseService : IPurchaseService
{
    ...
    private readonly IProcurementSyncService _sync;

    public LocalPurchaseService(IDbContextFactory<LocalAppDbContext> factory, IAppSessionContext session,
        IConnectivityService connectivity, IProcurementSyncService sync)
    ...
    public async Task<(bool Ok,string? Error)> CreateAsync(PurchaseModel m)
    {
        ...
        LocalSyncQueueWriter.Enqueue(db,"Purchase",row.Id,row.CompanyId,SyncOperation.Create,Payload(row),now); await db.SaveChangesAsync();
        _connectivity.IncrementPending(); TriggerSync(); return(true,null);
    }
    ...
    public async Task<(bool Ok,string? Error)> UpdateAsync(PurchaseModel m)
    {
        ...
        Apply(row,m,DateTime.UtcNow); LocalSyncQueueWriter.Enqueue(db,"Purchase",row.Id,row.CompanyId,SyncOperation.Update,Payload(row),row.UpdatedAtUtc); await db.SaveChangesAsync();
        _connectivity.IncrementPending(); TriggerSync(); return(true,null);
    }

    public async Task<(bool Ok,string? Error)> DeleteAsync(Guid id)
    {
        await using var db=await _factory.CreateDbContextAsync(); var row=await db.Purchases.FirstOrDefaultAsync(x=>x.Id==id);
        if(row is null || !Allows(row.CompanyId)) return(false,"Purchase not found.");
        db.Purchases.Remove(row); LocalSyncQueueWriter.Enqueue(db,"Purchase",row.Id,row.CompanyId,SyncOperation.Delete,new{row.Id,row.CompanyId},DateTime.UtcNow); await db.SaveChangesAsync();
        _connectivity.IncrementPending(); TriggerSync(); return(true,null);
    }
    ...
    private void TriggerSync(){if(_connectivity.IsOnline)_=_sync.SyncPendingAsync();}
    ...
    private static object Payload(LocalPurchaseEntity x)=>new{x.Id,x.CompanyId,x.PurchaseNumber,x.SupplierId,x.SupplierName,x.PurchaseDate,x.DueDate,x.ItemsJson,x.PaymentMethod,x.Reference,x.Notes,x.Status,x.PaidAmount,SyncVersion=1L};
