...
public sealed class LocalPurchaseService : IPurchaseService
{
    ...
    public async Task<PurchaseModel?> GetByIdAsync(Guid id)
    {
        ...
        await using var db = await _factory.CreateDbContextAsync();
        var row = await db.Purchases.AsNoTracking().Include(x => x.Items).FirstOrDefaultAsync(x => x.Id == id && !x.IsDeleted);
    ...
    public async Task<(bool Ok, string? Error)> CreateAsync(PurchaseModel purchase)
    {
        ...
        try
        {
            ...
            if (AffectsStock(purchase.Status))
                foreach (var product in products)
                    ApplyStockChange(db, purchase.CompanyId, product, qty[product.Id], purchase.PurchaseNumber, "Purchase received", now);
    ...
    public async Task<(bool Ok, string? Error)> UpdateAsync(PurchaseModel purchase)
    {
        ...
        if ((PurchaseStatus)entity.Status == PurchaseStatus.Cancelled) return (false, "Cancelled purchases cannot be edited.");
        await using var tx = await db.Database.BeginTransactionAsync();
        try
        {
            var now = DateTime.UtcNow;
            var oldQty = AffectsStock((PurchaseStatus)entity.Status)
                ? entity.Items.Where(x => !x.IsDeleted).GroupBy(x => x.ProductId).ToDictionary(g => g.Key, g => g.Sum(x => x.Quantity))
            ...
            foreach (var product in products)
            {
                ...
                if (stockDelta < 0 && product.Stock < -stockDelta)
                    return (false, $"Cannot reduce received quantity for {product.Name}; current stock is too low.");
            }
            foreach (var product in products)
            {
                var stockDelta = newQty.GetValueOrDefault(product.Id) - oldQty.GetValueOrDefault(product.Id);
                if (stockDelta == 0) continue;
                ApplyStockChange(db, entity.CompanyId, product, stockDelta, purchase.PurchaseNumber,
                    stockDelta > 0 ? "Purchase received / edited" : "Purchase receipt reversed", now);
            }
            ...
            entity.UpdatedAt = now; entity.SyncStatus = (int)SyncStatus.PendingSync;
    ...
    {
        if (id == Guid.Empty) return (false, "Purchase is required.");
        await using var db = await _factory.CreateDbContextAsync();
        var entity = await db.Purchases.Include(x => x.Items).FirstOrDefaultAsync(x => x.Id == id && !x.IsDeleted);
        if (entity == null) return (false, "Purchase not found.");
        await using var tx = await db.Database.BeginTransactionAsync();
        try
        {
            ...
            if (AffectsStock((PurchaseStatus)entity.Status))
            {
                ...
                if (products.Count != qty.Count) return (false, "One or more purchase products were not found.");
                foreach (var product in products)
                    if (product.Stock < qty[product.Id]) return (false, $"Cannot delete this received purchase because {product.Name} stock has already been used.");
                foreach (var product in products)
                    ApplyStockChange(db, entity.CompanyId, product, -qty[product.Id], entity.PurchaseNumber, "Purchase deleted / receipt reversed", now);
            }
        ...
        {
            await tx.RollbackAsync();
            System.Diagnostics.Debug.WriteLine($"[AveroNova] Offline purchase delete failed: {ex}");
            return (false, "Unable to delete purchase locally.");
        }
    }

    public async Task<string> GetNextPurchaseNumberAsync(Guid companyId)
    {
        if (companyId == Guid.Empty) return $"PO-{DateTime.Today:yyyy}-0001";
        await using var db = await _factory.CreateDbContextAsync();
        var prefix = $"PO-{DateTime.Today:yyyy}-";
        var numbers = await db.Purchases.AsNoTracking()
            .Where(x => x.CompanyId == companyId && x.PurchaseNumber.StartsWith(prefix))
            .Select(x => x.PurchaseNumber).ToListAsync();
    ...
    {
        if (requireId && p.LocalId == Guid.Empty) return "Purchase is required.";
        if (p.CompanyId == Guid.Empty) return "Company is required.";
        if (string.IsNullOrWhiteSpace(p.SupplierName)) return "Supplier is required.";
        if (p.Items.Count == 0) return "Add at least one line item.";
        if (p.Items.Any(x => x.ProductId == Guid.Empty || x.Quantity <= 0 || x.UnitPrice < 0)) return "Purchase contains an invalid line item.";
        if (p.PaidAmount < 0 || p.PaidAmount > p.GrandTotal) return "Paid amount is invalid.";
        return null;
    }
    ...
    private static void ApplyStockChange(AppDbContext db, Guid companyId, Product product, int signedChange, string reference, string notes, DateTime now)
    {
        ...
        {
            Id = Guid.NewGuid(), CompanyId = companyId, ProductId = product.Id,
            MovementType = (int)(signedChange >= 0 ? StockMovementType.In : StockMovementType.Out),
            Quantity = signedChange, StockBefore = before, StockAfter = product.Stock,
            Reference = reference, Notes = notes, CreatedBy = "Offline ERP",
    ...
    private static PurchaseModel Map(Purchase entity) => new()
    {
        ...
        {
            ProductId = x.ProductId, ProductName = x.ProductName, SKU = x.SKU,
            UnitPrice = x.UnitPrice, Quantity = x.Quantity, TaxPct = x.TaxPct
        }).ToList()
    };
}
