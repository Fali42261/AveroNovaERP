...
public sealed class LocalBillingService : IBillingService
{
    ...
    public async Task<List<InvoiceModel>> GetAllAsync(Guid companyId)
    {
        ...
        var rows = await db.Invoices.AsNoTracking().Include(x => x.Items).Where(x => x.CompanyId == companyId && !x.IsDeleted).OrderByDescending(x => x.InvoiceDate).ThenByDescending(x => x.CreatedAt).ToListAsync();
    ...
    public async Task<(bool Ok, string? Error)> CreateAsync(InvoiceModel invoice)
    {
        ...
        try
        {
            ...
            if (AffectsStock(invoice.Status))
                foreach (var product in products) ApplyStockChange(db, invoice.CompanyId, product, -required[product.Id], StockMovementType.Out, invoice.InvoiceNumber, "Sale", now);
            await db.SaveChangesAsync(); await tx.CommitAsync(); invoice.LocalId = entity.Id; return (true, null);
        }
        catch (Exception ex) { await tx.RollbackAsync(); System.Diagnostics.Debug.WriteLine($"[AveroNova] Offline invoice create failed: {ex}"); return (false, "Unable to save invoice locally."); }
    }
    ...
    public async Task<(bool Ok, string? Error)> UpdateAsync(InvoiceModel invoice)
    {
        ...
        try
        {
            ...
            entity.Items = invoice.Items.Select(x => new InvoiceItem { Id = Guid.NewGuid(), InvoiceId = entity.Id, ProductId = x.ProductId, ProductName = x.ProductName, SKU = x.SKU, UnitPrice = x.UnitPrice, Quantity = x.Quantity, DiscountPct = x.DiscountPct, TaxPct = x.TaxPct, CreatedAt = now, UpdatedAt = now, IsDeleted = false }).ToList();
            await db.SaveChangesAsync(); await tx.CommitAsync(); invoice.SyncStatus = SyncStatus.PendingSync; return (true, null);
        }
        catch (Exception ex) { await tx.RollbackAsync(); System.Diagnostics.Debug.WriteLine($"[AveroNova] Offline invoice update failed: {ex}"); return (false, "Unable to update invoice locally."); }
    }

    public async Task<(bool Ok, string? Error)> DeleteAsync(Guid id)
    {
        if (id == Guid.Empty) return (false, "Invoice is required.");
        await using var db = await _factory.CreateDbContextAsync(); var entity = await db.Invoices.Include(x => x.Items).FirstOrDefaultAsync(x => x.Id == id && !x.IsDeleted);
        if (entity == null) return (false, "Invoice not found."); await using var tx = await db.Database.BeginTransactionAsync();
        try
        {
            var now = DateTime.UtcNow;
        ...
        catch (Exception ex) { await tx.RollbackAsync(); System.Diagnostics.Debug.WriteLine($"[AveroNova] Offline invoice delete failed: {ex}"); return (false, "Unable to delete invoice locally."); }
    }
    ...
    public async Task<(bool Ok, string? Error)> CancelAsync(Guid id)
    {
        ...
        catch (Exception ex) { await tx.RollbackAsync(); System.Diagnostics.Debug.WriteLine($"[AveroNova] Offline invoice cancel failed: {ex}"); return (false, "Unable to cancel invoice locally."); }
    }

    public async Task<string> GetNextInvoiceNumberAsync(Guid companyId)
    {
        await using var db = await _factory.CreateDbContextAsync(); var count = await db.Invoices.CountAsync(x => x.CompanyId == companyId && !x.IsDeleted);
        return $"INV-{DateTime.Today:yyyy}-{count + 1:D4}";
    }

    public async Task<List<InvoiceModel>> GetByCustomerAsync(Guid customerId)
    {
        await using var db = await _factory.CreateDbContextAsync(); var rows = await db.Invoices.AsNoTracking().Include(x => x.Items).Where(x => x.CustomerId == customerId && !x.IsDeleted).OrderByDescending(x => x.InvoiceDate).ToListAsync();
        return rows.Select(Map).ToList();
    }
    ...
    public async Task<List<InvoiceModel>> GetOverdueAsync(Guid companyId)
    {
        ...
        var rows = await db.Invoices.AsNoTracking().Include(x => x.Items).Where(x => x.CompanyId == companyId && !x.IsDeleted && x.DueDate < today && x.Status != (int)InvoiceStatus.Paid && x.Status != (int)InvoiceStatus.Cancelled).OrderBy(x => x.DueDate).ToListAsync();
        return rows.Select(Map).ToList();
    }
    ...
    private static string? Validate(InvoiceModel invoice, bool requireId = false)
    {
        ...
        if (invoice.CompanyId == Guid.Empty) return "Company is required."; if (invoice.CustomerId == Guid.Empty) return "Customer is required.";
        ...
        if (invoice.PaidAmount < 0 || invoice.PaidAmount > invoice.GrandTotal) return "Paid amount is invalid."; return null;
    }
    ...
    {
        Id = model.LocalId, CompanyId = model.CompanyId, InvoiceNumber = model.InvoiceNumber, CustomerId = model.CustomerId, CustomerName = model.CustomerName, InvoiceDate = model.InvoiceDate,
        DueDate = model.DueDate, DiscountPct = model.DiscountPct, TaxPct = model.TaxPct, PaymentMethod = (int)model.PaymentMethod, PaidAmount = model.PaidAmount, Notes = model.Notes,
        Status = (int)model.Status, SyncStatus = (int)SyncStatus.PendingSync, CreatedAt = now, UpdatedAt = now, IsDeleted = false,
        Items = model.Items.Select(x => new InvoiceItem { Id = Guid.NewGuid(), InvoiceId = model.LocalId, ProductId = x.ProductId, ProductName = x.ProductName, SKU = x.SKU, UnitPrice = x.UnitPrice, Quantity = x.Quantity, DiscountPct = x.DiscountPct, TaxPct = x.TaxPct, CreatedAt = now, UpdatedAt = now, IsDeleted = false }).ToList()
    };

    private static InvoiceModel Map(Invoice entity) => new()
    {
        LocalId = entity.Id, CompanyId = entity.CompanyId, InvoiceNumber = entity.InvoiceNumber, CustomerId = entity.CustomerId, CustomerName = entity.CustomerName, InvoiceDate = entity.InvoiceDate,
        ...
        PaidAmount = entity.PaidAmount, Notes = entity.Notes, Status = Enum.IsDefined(typeof(InvoiceStatus), entity.Status) ? (InvoiceStatus)entity.Status : InvoiceStatus.Draft,
        ...
        Items = entity.Items.Where(x => !x.IsDeleted).Select(x => new InvoiceLineItem { ProductId = x.ProductId, ProductName = x.ProductName, SKU = x.SKU, UnitPrice = x.UnitPrice, Quantity = x.Quantity, DiscountPct = x.DiscountPct, TaxPct = x.TaxPct }).ToList()
    };
}
