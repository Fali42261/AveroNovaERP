...
public sealed class LocalBillingService : IBillingService
{
    ...
    public LocalBillingService(IDbContextFactory<AppDbContext> factory)
    {
        _factory = factory;
    }
    ...
    public async Task<(bool Ok, string? Error)> UpdateAsync(InvoiceModel invoice)
    {
        ...
        await using var db = await _factory.CreateDbContextAsync();
        var entity = await db.Invoices.Include(x => x.Items)
            .FirstOrDefaultAsync(x => x.Id == invoice.LocalId && !x.IsDeleted);
        ...
        try
        {
            ...
            entity.DiscountPct = invoice.DiscountPct;
            entity.TaxPct = invoice.TaxPct;
            entity.PaymentMethod = (int)invoice.PaymentMethod;
            entity.PaidAmount = invoice.PaidAmount;
            entity.Notes = invoice.Notes;
            entity.Status = (int)invoice.Status;
            entity.UpdatedAt = DateTime.UtcNow;
            entity.SyncStatus = (int)SyncStatus.PendingSync;
    ...
    public async Task<(bool Ok, string? Error)> CancelAsync(Guid id)
    {
        ...
        var entity = await db.Invoices.FirstOrDefaultAsync(x => x.Id == id && !x.IsDeleted);
        if (entity == null) return (false, "Invoice not found.");
        entity.Status = (int)InvoiceStatus.Cancelled;
        entity.UpdatedAt = DateTime.UtcNow;
        entity.SyncStatus = (int)SyncStatus.PendingSync;
        await db.SaveChangesAsync();
        return (true, null);
