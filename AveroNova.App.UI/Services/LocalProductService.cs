...
public sealed class LocalProductService : IProductService
{
    ...
    public LocalProductService(IDbContextFactory<LocalAppDbContext> dbFactory, IAppSessionContext session,
        ISyncService sync, IConnectivityService connectivity)
    {
    ...
    public async Task<List<ProductModel>> GetAllAsync(Guid companyId)
    {
        ...
        var rows = await db.Products.AsNoTracking().Where(p => p.CompanyId == companyId)
    ...
    {
        if (!Allows(product.CompanyId)) return (false, "You do not have access to this company.");
        if (string.IsNullOrWhiteSpace(product.Name)) return (false, "Product name is required.");
    ...
    public async Task<(bool Ok, string? Error)> UpdateAsync(ProductModel product)
    {
        ...
        if (row is null || !Allows(row.CompanyId)) return (false, "Product not found.");
    ...
    public async Task<(bool Ok, string? Error)> DeleteAsync(Guid id)
    {
        ...
        LocalSyncQueueWriter.Enqueue(db, "Product", row.Id, row.CompanyId, SyncOperation.Delete,
            new { row.Id, row.CompanyId }, DateTime.UtcNow);
        await db.SaveChangesAsync();
        _connectivity.IncrementPending();
        TriggerSyncIfOnline();
        return (true, null);
    ...
    private static ProductModel Map(LocalProductEntity row) => new()
    {
        LocalId = row.Id, ServerId = row.ServerId?.ToString("D"), CompanyId = row.CompanyId, Name = row.Name,
        SKU = row.SKU, Barcode = row.Barcode, Category = row.Category, Brand = row.Brand, Unit = row.Unit,
        PurchasePrice = row.PurchasePrice, SellingPrice = row.SellingPrice, TaxPercent = row.TaxPercent,
        Stock = row.Stock, MinimumStock = row.MinimumStock, Description = row.Description,
        Status = (ProductStatus)row.Status, CreatedAt = row.CreatedAtUtc, UpdatedAt = row.UpdatedAtUtc,
        LastSyncedAt = row.LastSyncedAtUtc, SyncStatus = ToUiStatus(row.SyncStatus)
    };

    private static LocalProductEntity ToEntity(ProductModel model, DateTime now) => new()
    {
        Id = model.LocalId, CompanyId = model.CompanyId, Name = model.Name.Trim(), SKU = model.SKU.Trim(),
        Barcode = model.Barcode, Category = model.Category, Brand = model.Brand,
        Unit = string.IsNullOrWhiteSpace(model.Unit) ? "pcs" : model.Unit, PurchasePrice = model.PurchasePrice,
        SellingPrice = model.SellingPrice, TaxPercent = model.TaxPercent, Stock = model.Stock,
        MinimumStock = model.MinimumStock, Description = model.Description, Status = (int)model.Status,
        CreatedAtUtc = now, UpdatedAtUtc = now
    };

    private static void Apply(LocalProductEntity row, ProductModel model, DateTime now)
    {
        row.Name = model.Name.Trim(); row.SKU = model.SKU.Trim(); row.Barcode = model.Barcode;
        row.Category = model.Category; row.Brand = model.Brand; row.Unit = string.IsNullOrWhiteSpace(model.Unit) ? "pcs" : model.Unit;
        row.PurchasePrice = model.PurchasePrice; row.SellingPrice = model.SellingPrice; row.TaxPercent = model.TaxPercent;
        row.Stock = model.Stock; row.MinimumStock = model.MinimumStock; row.Description = model.Description;
        row.Status = (int)model.Status; row.UpdatedAtUtc = now; row.SyncError = null;
    }

    private static object Payload(LocalProductEntity row) => new
    {
        row.Id, row.CompanyId, row.Name, Sku = row.SKU, row.Barcode, row.Category, row.Brand, row.Unit,
        row.PurchasePrice, row.SellingPrice, row.TaxPercent, row.Stock, row.MinimumStock, row.Description,
        row.Status, SyncVersion = 1L
    };
