...
public partial class ProductsListPage : ContentPage
{
    ...
    private void ApplyHeaderLayout(bool compact)
    {
        if (compact)
        {
            ...
            PageHeader.RowDefinitions = new RowDefinitionCollection { new RowDefinition(GridLength.Auto), new RowDefinition(GridLength.Auto) };
            Grid.SetColumn(HeaderTitle, 0); Grid.SetRow(HeaderTitle, 0); return;
        }
        PageHeader.ColumnDefinitions = new ColumnDefinitionCollection { new ColumnDefinition(GridLength.Star), new ColumnDefinition(GridLength.Auto) };
        PageHeader.RowDefinitions = new RowDefinitionCollection { new RowDefinition(GridLength.Auto) };
        Grid.SetColumn(HeaderTitle, 0); Grid.SetRow(HeaderTitle, 0);
    }
    ...
    private void ApplyToolbarLayout(bool compact)
    {
        if (compact)
        {
            Grid.SetColumn(SearchHost, 0); Grid.SetRow(SearchHost, 0); Grid.SetColumnSpan(SearchHost, 2);
            ...
            Grid.SetColumn(StatusHost, 0); Grid.SetRow(StatusHost, 1); Grid.SetColumnSpan(StatusHost, 2);
        ...
        StatusHost.WidthRequest = StatusControlWidth; StatusHost.MinimumWidthRequest = 160; StatusHost.MaximumWidthRequest = 200; StatusHost.HorizontalOptions = LayoutOptions.Start;
    }
    ...
    private void OpenStatusDropdown()
    {
        ...
        foreach (var option in _list.StatusFilters)
        {
            ...
            var label = new Label { Text = option, FontSize = 13, LineBreakMode = LineBreakMode.TailTruncation, VerticalOptions = LayoutOptions.Center, InputTransparent = true, TextColor = Res("TextPrimary", Colors.Black) };
            var row = new Border { Padding = new Thickness(12, 0), HeightRequest = 40, MinimumHeightRequest = 40, StrokeThickness = 0, StrokeShape = new RoundRectangle { CornerRadius = 8 }, Content = label };
            row.SetAppThemeColor(Border.BackgroundColorProperty, selected ? Res("Gray100", Color.FromArgb("#F3F4F6")) : Colors.Transparent, selected ? Res("InputBackgroundDark", Color.FromArgb("#1E293B")) : Colors.Transparent);
            var captured = option; var tap = new TapGestureRecognizer(); tap.Tapped += (_, _) => SelectStatusFilter(captured); row.GestureRecognizers.Add(tap); StatusDropdownItems.Children.Add(row);
        }
        StatusDropdownLayer.IsVisible = true; Dispatcher.Dispatch(PositionStatusDropdown);
    }
    ...
    private void PositionStatusDropdown()
    {
        ...
        var width = StatusHost.Width > 1 ? StatusHost.Width : StatusControlWidth; width = Math.Max(160, width); StatusDropdown.WidthRequest = width;
#if WINDOWS
        if (StatusHost.Handler?.PlatformView is Microsoft.UI.Xaml.FrameworkElement field && Root.Handler?.PlatformView is Microsoft.UI.Xaml.UIElement root)
        {
            ...
            StatusDropdown.Margin = new Thickness(point.X, point.Y + height + 4, 0, 0); return;
        }
#endif
        double x = 0, y = 0; VisualElement? current = StatusHost;
    ...
    private async Task ShowFormAsync(Guid? id)
    {
        ...
        CloseStatusDropdown(); ListPanel.IsVisible = false; DetailsView.IsVisible = false; FormView.IsVisible = true; await _form.InitializeAsync(id);
    }
    private async Task ShowDetailsAsync(Guid id)
    {
        if (!_list.CanView) return;
        CloseStatusDropdown(); ListPanel.IsVisible = false; FormView.IsVisible = false; DetailsView.IsVisible = true; await _details.LoadAsync(id);
    }
    private void ShowList(bool reload) { FormView.IsVisible = false; DetailsView.IsVisible = false; ListPanel.IsVisible = true; if (reload) _ = _list.LoadAsync(); }
    private void RenderList() { if (!ListPanel.IsVisible || ListHost == null) return; ListHost.Children.Clear(); foreach (var product in _list.Items) ListHost.Children.Add(BuildProductRow(product)); }
    ...
    private View BuildProductRow(ProductModel product)
    {
        ...
        var tap = new TapGestureRecognizer(); tap.Tapped += async (_, _) => await ShowDetailsAsync(product.LocalId); grid.GestureRecognizers.Add(tap);
        return grid;
    }
