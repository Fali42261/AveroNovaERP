...
public partial class InvoiceFormPage : ContentPage
{
    ...
    private void OnProductChanged(object? sender, EventArgs e)
    {
        ...
        EntryQuantity.Text = "1";
        LblSelectedProduct.Text = $"{product.Name} • {product.SKU} • Selling price: {product.SellingPrice:C} • Available: {product.Stock}";
        LblSelectedProduct.IsVisible = true;
    }

    private void OnAddLineItem(object? sender, EventArgs e)
    {
        if (ProductPicker.SelectedIndex < 0 || ProductPicker.SelectedIndex >= _productList.Count)
        {
        ...
        var product = _productList[ProductPicker.SelectedIndex];
        if (!int.TryParse(EntryQuantity.Text, out var quantity) || quantity <= 0)
        {
    ...
    private void RenderLineItems()
    {
        ...
        foreach (var item in _lineItems)
        {
            ...
            name.Children.Add(new Label { Text = item.SKU, FontSize = 10, TextColor = Color.FromArgb("#64748B") });
            row.Add(name, 0, 0);
            ...
            row.Add(new Label { Text = item.UnitPrice.ToString("C"), FontSize = 13, HorizontalOptions = LayoutOptions.End, VerticalOptions = LayoutOptions.Center }, 2, 0);
            var totalCell = new HorizontalStackLayout { Spacing = 6, HorizontalOptions = LayoutOptions.End };
            totalCell.Children.Add(new Label { Text = item.GrandTotal.ToString("C"), FontSize = 13, FontAttributes = FontAttributes.Bold, VerticalOptions = LayoutOptions.Center });
    ...
    private async Task SaveAsync(InvoiceStatus status)
    {
        ...
        if (result.Ok) await Shell.Current.GoToAsync("..");
        else ShowError(result.Error ?? "Unable to save sale.");
    }
