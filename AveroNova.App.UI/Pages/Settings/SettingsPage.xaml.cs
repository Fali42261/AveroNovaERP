...
public partial class SettingsPage : ContentPage, IHostedPage
{
    ...
    public SettingsPage(ISettingsService service, IConnectivityService connectivity, IMainContentNavigator navigator)
    {
        InitializeComponent();
        _service = service;
    ...
    public async Task LoadForHostAsync()
    {
        try
        {
            ...
            _settings.RememberLogin = Microsoft.Maui.Storage.Preferences.Default.Get(RememberLoginPreferenceKey, _settings.RememberLogin);
            _settings.AutoSync = true;
            AppRegionalPreferences.Apply(_settings.Language,_settings.DateFormat,_settings.Currency,_settings.CurrencySymbol,_settings.TimeZone,persist:false);
        ...
        catch (Exception ex)
        {
            ...
            SettingsContent.Children.Add(new Label { Text = "Settings could not be loaded. Pull down to retry.", TextColor = Color.FromArgb("#DC2626"), HorizontalOptions = LayoutOptions.Center, Margin = new Thickness(0,30) });
        }
    }
    ...
    private void OnConnectivityChanged(object? sender, ConnectivityStatus status) => MainThread.BeginInvokeOnMainThread(() => { if (_offline is not null) _offline.IsToggled = status == ConnectivityStatus.Offline; });
    ...
    private void BuildContent()
    {
        ...
        try
        {
            ...
            _accent = new Entry { Text = string.IsNullOrWhiteSpace(_settings.AccentColor) ? "#2563EB" : _settings.AccentColor, HorizontalTextAlignment = TextAlignment.Center, WidthRequest = 210, TextColor = primaryText };
            _compact = new Switch { IsToggled = _settings.CompactMode };
            _theme.SelectedIndexChanged += OnThemeChanged;
            SettingsContent.Children.Add(Card("Appearance",surface,border,primaryText,secondaryText,Row("Theme",_theme,secondaryText),Row("Accent color",_accent,secondaryText),Row("Compact mode",_compact,secondaryText)));
            ...
            SettingsContent.Children.Add(Card("Regional",surface,border,primaryText,secondaryText,Row("Language",_language,secondaryText),Row("Date format",_date,secondaryText),Row("Currency",_currency,secondaryText),Row("Time zone",_timeZone,secondaryText)));
            ...
            _remember = new Switch { IsToggled = Microsoft.Maui.Storage.Preferences.Default.Get(RememberLoginPreferenceKey,_settings.RememberLogin) };
            SettingsContent.Children.Add(Card("Sync & Preferences",surface,border,primaryText,secondaryText,Row("Notifications",_notifications,secondaryText),Row("Offline mode (current status)",_offline,secondaryText),Row("Remember login",_remember,secondaryText)));
            SettingsContent.Children.Add(new Label { Text = "Sync is automatic: online changes sync automatically; offline changes wait locally until internet returns.", FontSize = 11, TextColor = secondaryText, HorizontalTextAlignment = TextAlignment.Center });

            _message = new Label { IsVisible = false, HorizontalTextAlignment = TextAlignment.Center, FontSize = 12 };
            SettingsContent.Children.Add(_message);
            _saveButton = new Button { Text = "Save Settings", HeightRequest = 46, CornerRadius = 10, BackgroundColor = SafeAccentColor(_settings.AccentColor), TextColor = Colors.White, FontAttributes = FontAttributes.Bold, HorizontalOptions = LayoutOptions.Fill, MaximumWidthRequest = 500 };
    ...
    private void OnThemeChanged(object? sender, EventArgs e)
    {
        ...
        ApplyTheme(_settings.Theme);
        BuildContent();
        ShowMessage("Theme applied. Tap Save Settings to keep it.",true);
    }
    ...
    private void OnRegionalChanged(object? sender, EventArgs e)
    {
        ...
        CaptureControls();
        AppRegionalPreferences.Apply(_settings.Language,_settings.DateFormat,_settings.Currency,_settings.CurrencySymbol,_settings.TimeZone,persist:false);
        ShowMessage("Regional preference applied. Tap Save Settings to keep it.",true);
    }
    ...
    private async void OnSaveClicked(object? sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!result.Ok) { ShowMessage(result.Error ?? "Settings could not be saved.",false); return; }
            Microsoft.Maui.Storage.Preferences.Default.Set(RememberLoginPreferenceKey,_settings.RememberLogin);
            AppRegionalPreferences.Apply(_settings.Language,_settings.DateFormat,_settings.Currency,_settings.CurrencySymbol,_settings.TimeZone,persist:true);
            ApplyTheme(_settings.Theme);
        ...
        {
            _saving = false;
            if (_saveButton is not null) { _saveButton.IsEnabled = true; _saveButton.Text = "Save Settings"; }
        }
    }
    ...
    private static void ApplyTheme(ThemeMode theme)
    {
        ...
        Microsoft.Maui.Controls.Application.Current.UserAppTheme = theme switch { ThemeMode.Light => AppTheme.Light, ThemeMode.Dark => AppTheme.Dark, _ => AppTheme.Unspecified };
    }
    private static bool IsDark(ThemeMode theme) => theme == ThemeMode.Dark || (theme == ThemeMode.System && Microsoft.Maui.Controls.Application.Current?.RequestedTheme == AppTheme.Dark);
    private void ShowMessage(string text,bool success) { if (_message is null) return; _message.Text = text; _message.TextColor = success ? Color.FromArgb("#059669") : Color.FromArgb("#DC2626"); _message.IsVisible = true; }
    private static string NormalizeAccent(string? value) { var text = value?.Trim() ?? string.Empty; if (text.Length == 6 && !text.StartsWith('#')) text = "#" + text; return string.IsNullOrWhiteSpace(text) ? "#2563EB" : text; }
    private static Color SafeAccentColor(string? value) { try { return Color.FromArgb(NormalizeAccent(value)); } catch { return Color.FromArgb("#2563EB"); } }
    private static int FindIndex(string[] values,string? value) { var i = Array.IndexOf(values,value ?? string.Empty); return i < 0 ? 0 : i; }
    private static int ClampIndex(int index,int length) => index < 0 || index >= length ? 0 : index;
    private static Picker MakePicker(IEnumerable<string> values,int index,Color textColor) => new() { ItemsSource = values.ToList(), SelectedIndex = index, HorizontalTextAlignment = TextAlignment.Center, WidthRequest = 210, TextColor = textColor };
    ...
    private static Border Card(string title,Color background,Color border,Color titleColor,Color dividerColor,params View[] rows)
    {
        ...
        return new Border { Stroke = border, StrokeThickness = 1, StrokeShape = new RoundRectangle { CornerRadius = new CornerRadius(14) }, BackgroundColor = background, Padding = new Thickness(16), MaximumWidthRequest = 760, Content = stack };
    }
}
