...
public partial class PurchaseReturnsListPage : ContentPage
{
    ...
    {
        var items = await _svc.GetPurchaseReturnsAsync(_company.CurrentCompany?.LocalId ?? Guid.Empty);
        List.Children.Clear();
        foreach (var r in items) List.Children.Add(BuildRow(r));
        if (items.Count == 0) List.Children.Add(new Label { Text = "No purchase returns.", FontSize = 14, TextColor = Color.FromArgb("#64748B"), HorizontalOptions = LayoutOptions.Center, Margin = new Thickness(0, 40) });
    }

    private View BuildRow(PurchaseReturnModel r)
    {
        ...
        left.Children.Add(new Label { Text = r.SupplierName, FontSize = 13, TextColor = Color.FromArgb("#64748B") });
        left.Children.Add(new Label { Text = $"PO: {r.PurchaseNumber}  •  {r.ReturnDate:dd MMM yyyy}", FontSize = 11, TextColor = Color.FromArgb("#94A3B8") });
