...
public partial class SalesReturnFormPage : ContentPage, IHostedPage
{
    ...
    private SalesReturnModel? _editing;
    ...
    private async void OnSaveClicked(object s,EventArgs e)
    {
        ...
        var invoice=_invoices[InvoicePicker.SelectedIndex];
        ...
        var(ok,err)=isNew?await _svc.CreateSalesReturnAsync(ret):await _svc.UpdateSalesReturnAsync(ret);
        if(!ok){await FailAsync(err??"Unable to save sales return.");return;}
        ErrorBanner.IsVisible=false;
        await AppToast.SuccessAsync(isNew?"Sales return created successfully.":"Sales return updated successfully.");
