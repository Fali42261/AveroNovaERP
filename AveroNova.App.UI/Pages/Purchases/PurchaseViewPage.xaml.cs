...
public partial class PurchaseViewPage : ContentPage
{
    ...
    {
        base.OnAppearing();
        if (!string.IsNullOrEmpty(PurchaseId) && Guid.TryParse(PurchaseId, out var id)) await LoadAsync(id);
    }
    ...
    private void BuildContent(PurchaseModel p)
    {
        ...
        Row("Status", p.StatusLabel); Row("Date", p.PurchaseDate.ToString("dd MMM yyyy")); Row("Due Date", p.DueDate.ToString("dd MMM yyyy"));
        Row("Reference", p.Reference); Row("Grand Total", $"₹{p.GrandTotal:N2}"); Row("Paid", $"₹{p.PaidAmount:N2}"); Row("Due", $"₹{p.DueAmount:N2}");
        if (!string.IsNullOrEmpty(p.Notes)) Row("Notes", p.Notes); card.Content = vsl;
        ...
        deleteBtn.Clicked += async (_, _) =>
        {
            ...
            var result = await _svc.DeleteAsync(p.LocalId);
            if (!result.Ok) { await DisplayAlertAsync("Unable to delete", result.Error ?? "Delete failed.", "OK"); return; }
            await CloseAsync();
        };
        Content.Children.Add(card); Content.Children.Add(deleteBtn);
    }
    ...
    {
        if (_purchase != null && EditRequested != null) await EditRequested(_purchase.LocalId);
    }
