...
public partial class CustomerFormPage : ContentPage
{
    ...
    public CustomerFormPage(ICustomerService svc, ICompanyService company)
    {
        InitializeComponent();
        _svc = svc;
        _company = company;
    ...
    private async void OnSaveClicked(object s, EventArgs e)
    {
        if (string.IsNullOrWhiteSpace(EntryName.Text))
        {
            ...
            await AppToast.ErrorAsync("Customer name is required.");
            return;
        }
        ...
        if (ok)
        {
            await AppToast.SuccessAsync(isNew ? "Customer created successfully." : "Customer updated successfully.");
            await Shell.Current.GoToAsync("..");
        }
        else
        {
            var message = err ?? "Save failed.";
            ShowError(message);
