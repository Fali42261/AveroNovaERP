...
public partial class CustomerViewPage : ContentPage
{
    ...
    private void BuildContent(CustomerModel c)
    {
        ...
        pGrid.Add(av, 0, 0); pGrid.Add(info, 1, 0); profileCard.Content = pGrid;
        ...
        AddDetail("Status", c.StatusLabel); AddDetail("Address", c.Address); AddDetail("City", c.City); AddDetail("Country", c.Country); AddDetail("Tax No.", c.TaxNumber);
        if (!string.IsNullOrEmpty(c.Notes)) AddDetail("Notes", c.Notes);
        detailCard.Content = dVsl;

        Content.Children.Add(profileCard); Content.Children.Add(statsCard); Content.Children.Add(detailCard);
    }
    ...
    private async void OnDeleteClicked(object s, EventArgs e)
    {
        ...
        if (!ok)
        {
            await AppToast.ErrorAsync(error ?? "Customer could not be deleted.");
            return;
        }
        await AppToast.SuccessAsync("Customer deleted successfully.");
