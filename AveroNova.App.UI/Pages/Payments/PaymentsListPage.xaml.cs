...
public partial class PaymentsListPage : ContentPage
{
    ...
    public PaymentsListPage(IPaymentService svc, ICompanyService company, IBillingService billing, IPurchaseService purchases)
    ...
    private View BuildRow(PaymentModel p)
    {
        ...
        var right = new VerticalStackLayout { Spacing = 6, HorizontalOptions = LayoutOptions.End }; right.Children.Add(new Label { Text = $"₹{p.Amount:N2}", FontSize = 16, FontAttributes = FontAttributes.Bold, TextColor = Color.FromArgb("#059669"), HorizontalOptions = LayoutOptions.End });
        var viewBtn = new Button { Text = "View", Style = TryStyle("SmallSecondaryButton"), HeightRequest = 36, Padding = new Thickness(14, 0) }; viewBtn.Clicked += async (_, _) => await OpenViewAsync(p.LocalId); right.Children.Add(viewBtn);
