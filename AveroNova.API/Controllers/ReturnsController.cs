...
public sealed class ReturnsController(AppDbContext db) : ControllerBase
{
    ...
    public async Task<IActionResult> CreateSales([FromBody] SalesReturnRequest r, CancellationToken ct)
    {
        ...
        var stockError = await ApplyStockDeltaAsync(r.CompanyId, returned, 1, r.ReturnNumber, "Sales return", now, ct);
    ...
    public async Task<IActionResult> UpdateSales(Guid id, [FromBody] SalesReturnRequest r, CancellationToken ct)
    {
        ...
        var now=DateTime.UtcNow; var stockError=await ApplyStockDifferenceAsync(row.CompanyId,oldItems,newItems,1,row.ReturnNumber,"Sales return updated",now,ct);
        if(stockError is not null)return BadRequest(new{success=false,error=stockError});
    ...
    public async Task<IActionResult> DeleteSales(Guid id, CancellationToken ct)
    {
        ...
        TryReadReturnItems(row.ItemsJson,out var items,out _);var now=DateTime.UtcNow;var stockError=await ApplyStockDeltaAsync(row.CompanyId,items,-1,row.ReturnNumber,"Sales return deleted",now,ct);if(stockError is not null)return BadRequest(new{success=false,error=stockError});
    ...
    public async Task<IActionResult> CreatePurchase([FromBody] PurchaseReturnRequest r, CancellationToken ct)
    {
        ...
        TryReadReturnItems(r.ItemsJson,out var returned,out _);var now=DateTime.UtcNow;var stockError=await ApplyStockDeltaAsync(r.CompanyId,returned,-1,r.ReturnNumber,"Purchase return",now,ct);if(stockError is not null)return BadRequest(new{success=false,error=stockError});
    ...
    public async Task<IActionResult> UpdatePurchase(Guid id,[FromBody] PurchaseReturnRequest r,CancellationToken ct)
    {
        ...
        TryReadReturnItems(row.ItemsJson,out var oldItems,out _);TryReadReturnItems(r.ItemsJson,out var newItems,out _);var now=DateTime.UtcNow;var stockError=await ApplyStockDifferenceAsync(row.CompanyId,oldItems,newItems,-1,row.ReturnNumber,"Purchase return updated",now,ct);if(stockError is not null)return BadRequest(new{success=false,error=stockError});
    ...
    public async Task<IActionResult> DeletePurchase(Guid id,CancellationToken ct)
    {
        ...
        TryReadReturnItems(row.ItemsJson,out var items,out _);var now=DateTime.UtcNow;var stockError=await ApplyStockDeltaAsync(row.CompanyId,items,1,row.ReturnNumber,"Purchase return deleted",now,ct);if(stockError is not null)return BadRequest(new{success=false,error=stockError});
        row.IsDeleted=true; row.MarkPendingChange(); row.SyncStatus=RecordSyncStatus.Synced; row.LastSyncedAt=now; await db.SaveChangesAsync(ct); return Ok(new{success=true});
    }
    ...
    private async Task<string?> ValidatePurchase(PurchaseReturnRequest r,CancellationToken ct)
    {
        ...
        var purchaseTotal=source.Sum(x=>(x.UnitPrice*x.Quantity)*(1+x.TaxPct/100m));if(r.RefundAmount>purchaseTotal)return "Refund cannot exceed purchase total.";return null;
    }
